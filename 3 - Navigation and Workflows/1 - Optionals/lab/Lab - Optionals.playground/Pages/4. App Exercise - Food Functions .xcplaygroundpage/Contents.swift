...
func yourMeal(chosenMeal:String)->String?{
    if meals[chosenMeal] != nil{
        let foods=meals[chosenMeal]!
        let displaytoUser=("This meal includes\(foods.food[0],foods.food[1],foods.food[2]) and it has \(foods.calories) calories")
...
let chosenMeal:String
if let unWrappedTodaysMeal=yourMeal(chosenMeal: "Breakfast"){
    print(unWrappedTodaysMeal)
}else{
    print("This meal does not exist or hasn't been logged")
}
